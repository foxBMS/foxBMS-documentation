...
The ``can_cfg.c`` file is responsible for the general CAN options (CAN bit-rate, sending behaviour). The received messages are to be defined here.
...
Moreover the settings for message reception need to be set in the source file. The received messages are defined in the ``CAN_MSG_RX_TYPE_s can_RxMsgs[CAN_NUMBER_OF_RX_IDs]`` array structs:
