...
During the initialization, the driver will check and update the data of the selected channels as shown in :numref:`Fig. %s <nvm_channelinit>`. The specific controll mechanisms to identify corrupt data are:
...
(2) When creating the structure, users have to pay attention to the correct data allignment. Data should be stored in blocks of 4 byte (32 bit) to avoid errors when reading or writing data.
