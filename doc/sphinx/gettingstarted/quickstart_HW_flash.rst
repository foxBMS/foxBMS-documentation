...
This section describes how to supply the |foxBMS| hardware in order to flash compiled sources.
...
The first step is to supply the |Master|, which works with supply voltages between 12 and 24V DC.
To supply the |Master|, a connector must be prepared for the ``Supply`` connector as shown
...
To power up the |Master|, plug in the supply connector and apply a
voltage between 12 and 24V. SUPPLY_EXT_0 / GND_EXT_0 and SUPPLY_EXT_2 / GND_EXT_2 may be connected to the
same source for this initial test. At this point, the |Master| should draw approximately 150mA at 12V or 110mA at 24V. 

-------------------------
Primary and Secondary MCU
-------------------------

The |BMS-Master| has two MCUs: primary (|MCU0|) and secondary (|MCU1|). The secondary MCU is present to ensure redundant safety.
...
.. note::
      ...
      Before the connection is made between the |Master| and the computer for the first time, the computer must be connected to the internet,
...
must be used to connect the |Master| to a PC. As for the primary MCU, the PC must be connected to the internet before the connection
is made with the |Master|.
...
As the archive |primary archive| was used, the sources are made for the |MCU0|: the |MCU0| will be
flashed first.
...
The device must be connected to the computer with the USB cable. The |MCU0| must be used.
In the :ref:`hw_quickstart_cables`, it is explained how to find the |MCU0| and where the primary USB connector
is located.
...
With the supplied code, the |MCU1| checks the temperatures and voltages. If one or more of the values
are outside the limits, the |MCU1| opens the interlock line (thus opening the contactors). The interlock
line remains open until the |MCU1| is reset.
